...
The PostgreSQL server uses standard username/password authentication with credentials securely managed in Azure Key Vault.

**How it works:**
- Admin username (`psqladmin`) and password are created during deployment
- Password is auto-generated by Terraform (32 characters, high entropy)
- Both password and full connection string are stored in Azure Key Vault
- Applications use their managed identity to retrieve credentials from Key Vault at startup
- No passwords are stored in application code or configuration files

**Security benefits:**
- ✅ Credentials never exposed in source code or container images
- ✅ Key Vault provides encryption at rest and audit logging
- ✅ Managed identities eliminate the need for service principal credentials
- ✅ Private network access only (no public endpoint)
- ✅ Simple, proven authentication model with excellent security posture

**Password rotation:**
- Recommended every 90 days for production environments
- Process: Update password in Azure Portal → Update Key Vault secret → Restart applications
- Rotation can be automated using Azure Key Vault secret rotation policies if required
